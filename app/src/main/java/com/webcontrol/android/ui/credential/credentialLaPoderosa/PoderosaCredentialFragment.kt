...
import com.webcontrol.android.common.DocumentAdapterPoderosa
...
import com.webcontrol.android.databinding.FragmentCredentialPoderosaBinding
...
@AndroidEntryPoint
class PoderosaCredentialFragment : Fragment() {
    private lateinit var binding: FragmentCredentialPoderosaBinding
    private val viewModel by sharedViewModel<PoderosaCredentialViewModel>()
    lateinit var worker: WorkerLaPoderosa
    ...
    var accessDocuments: List<DocumentLaPoderosa> = ArrayList()
    var driverDocuments: List<DocumentLaPoderosa> = ArrayList()
    ...
    override fun onCreateView(
        ...
        binding = FragmentCredentialPoderosaBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    private fun loadDataUI(worker: WorkerLaPoderosa) {
        ...
        binding.includefrontq.txtAutorizadoAcceso.text =
            transformarAutorizacion(worker.autorizadoAcceso ?: "", "Acceso")
        ...
        binding.includeback.lblItemsZonaConduccion.text =
            (worker.zonasCond ?: "").replace(",", "\n")
        if (obtenerArreglo(
                (worker.tiposVehi ?: "")
            ) < 4
        ) binding.includeback.btnTipoVehiculo.visibility =
    ...
    private fun transformarAutorizacion(authorization: String, tipo: String): String {
        ...
        if (authorization.equals("APROBADO")) {
            authorizationAux = "SI"
            if (tipo == "Acceso") binding.includefrontq.txtAutorizadoAcceso.setTextColor(
                resources.getColor(
    ...
    private fun dialog(Subtitulo: String, items: List<DocumentLaPoderosa>) {
        val dialog = Dialog(requireContext())
        ...
        val adapter = DocumentAdapterPoderosa(items)
