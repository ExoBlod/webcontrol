...
import com.webcontrol.android.databinding.FragmentCredentialPhcBinding
import com.webcontrol.android.util.GlideApp
import com.webcontrol.android.util.RestClient
import com.webcontrol.android.util.SharedUtils
import dagger.hilt.android.AndroidEntryPoint
import kotlinx.android.synthetic.main.dialog_vigencia_documentos_accesos.*
import retrofit2.Call
import retrofit2.Callback
import retrofit2.Response

@AndroidEntryPoint
class PacificHydroCredential : Fragment() {
    private lateinit var binding: FragmentCredentialPhcBinding
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        binding = FragmentCredentialPhcBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    private fun loadData() {
        ...
        call.enqueue(object : Callback<WorkerCredentialPHC> {
            override fun onFailure(call: Call<WorkerCredentialPHC>, t: Throwable) {
                isValid = false
                SharedUtils.showToast(requireContext(), "Error obteniendo credenciales")
                SharedUtils.dismissLoader(requireContext())
            }

            override fun onResponse(call: Call<WorkerCredentialPHC>,
                                    response: Response<WorkerCredentialPHC>
            ) {
                    ...
                    if (worker != null) {
                        ...
                        binding.includefrontq.viewHeaderFront.background =
                            ResourcesCompat.getDrawable(
                                resources,
                                R.color.colorCredentialGoldfieldsMand,
                                null
                            )
                        binding.includeback.viewHeaderBack.background = ResourcesCompat.getDrawable(
                            resources,
                            R.color.colorCredentialGoldfieldsMand,
                            null
                        )

                        binding.includefrontq.lblEmpresa.text = worker.workerEmpresa
                        ...
                        binding.includefrontq.lblAutorizado.text = worker.workerAutorizado
                        binding.includeback.lblVencLic.text =
                            if (worker.workerFlicconducir.isNullOrEmpty()) getString(
                                R.string.credential_default_data
                            ) else SharedUtils.getNiceDate(worker.workerFlicconducir)
                        binding.includeback.lblLicMunicipal.text =
                            if (worker.WorkerMunicipal.isNullOrEmpty()) getString(
                                R.string.credential_default_data
                            ) else worker.WorkerMunicipal
                        binding.includeback.lblVencPsico.text =
                            if (worker.workerPsico.isNullOrEmpty()) getString(
                                R.string.credential_default_data
                            ) else SharedUtils.getNiceDate(worker.workerPsico)
                        binding.includeback.lblFecIngresoComp.text =
                            SharedUtils.getNiceDate(worker.fechain)
                    }else {
                        isValid = false
