...
import com.webcontrol.android.ui.covid.declaracion.DJOMAdapter
import com.webcontrol.android.ui.covid.declaracion.DJTRPEAdapter
...
class DJDetailAdapter(private val context: Context, testListResponse: ArrayList<DJRespuesta>)
    ...
    private fun onBindTXT(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        txtHolder.setUpView(cuestionario)
    }

    private fun onBindYesNo(holder: RecyclerView.ViewHolder, position: Int) {
        val yesNoHolder = holder as YesNoViewHolder
        val cuestionario = getItem(position)
        yesNoHolder.yesNoListener.updateCuestionario(cuestionario)
        yesNoHolder.setUpView(cuestionario = cuestionario)
    }
    ...
    private fun onBindOM(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        pregunta?.let {
            it.alternativas?.let { alternativas ->
                ...
                holder.itemView.rcv_answers.apply {
                    layoutManager = childLayoutManager
                    adapter = itemAdapter
                    setRecycledViewPool(viewPool)
                }
    ...
    inner class DATEViewHolder(view: View): RecyclerView.ViewHolder(view) {
        ...
        fun setUpView(cuestionario: DJPregunta?) {
            cuestionario?.let {
                testTitleTextView.text = "${it.orden}. ${it.descripcion}"
                ...
                testAnswer.setOnClickListener {
                    val calendarMaxDate: Calendar = Calendar.getInstance()
                    calendarMaxDate.add(Calendar.DATE, +1)
                    val dialog = DatePickerDialog(context, setOnDatePickListener(testAnswer, calendar),
                            calendar.get(Calendar.YEAR),
                            calendar.get(Calendar.MONTH),
                            calendar.get(Calendar.DAY_OF_MONTH)
                    )
                    dialog.datePicker.maxDate = calendarMaxDate.timeInMillis
                    dialog.show()
    ...
    inner class TXTViewHolder(view: View, listener: TXTEditTextListener): RecyclerView.ViewHolder(view) {
        ...
        fun setUpView(cuestionario: DJPregunta?) {
            cuestionario?.let {
                ...
                testAnswer.setText("")
                getResponseItem(cuestionario.id)?.respuesta?.let {str ->
                    testAnswer.setText(str)
                }
            }
        }
    }

    inner class YesNoViewHolder(view: View, listener: YESNOClickListener): RecyclerView.ViewHolder(view) {

        private val testTitleTextView: TextView = view.lblTestTitle
        private val btnRight: Button = view.sekizbit_switch_btn_right
        private val btnLeft: Button = view.sekizbit_switch_btn_left
        val yesNoListener: YESNOClickListener = listener

        init {
            yesNoListener.setBtnLeft(btnLeft)
            yesNoListener.setBtnRight(btnRight)

            btnRight.setOnClickListener(yesNoListener)
            btnLeft.setOnClickListener(yesNoListener)
        }
    ...
    inner class TRAVViewHolder(view: View): RecyclerView.ViewHolder(view) {
        ...
        fun setUpView(cuestionario: DJPregunta?) {
            cuestionario?.let {
                // state
                testTitleSwitch.text = "${it.orden}. ${it.descripcion}"
        ...
        fun setUpView(cuestionario: DJPregunta?, itemAdapter: DJTRPEAdapter) {
            cuestionario?.let {
                ...
                fechaRetorno.setOnClickListener {
                    val dialog = DatePickerDialog(context, setOnDatePickListener(fechaRetorno, calendar),
                            calendar.get(Calendar.YEAR),
                            calendar.get(Calendar.MONTH),
                            calendar.get(Calendar.DAY_OF_MONTH)
                    )
    ...
    inner class OMViewHolder(view: View): RecyclerView.ViewHolder(view) {
        ...
        fun setUpView(cuestionario: DJPregunta?) {
            cuestionario?.let {pregunta ->
                testTitleTextView.text = "${pregunta.orden}. ${pregunta.descripcion}"
                getResponseItem(adapterPosition)?.let {
                    if (it.tipo == "OMU" && it.respuesta != "NO")
    ...
    inner class YESNOClickListener: View.OnClickListener {

        private var cuestionario: DJPregunta? = null
        private var btnRight: Button? = null
        private var btnLeft: Button? = null


        fun updateCuestionario(_cuestionario: DJPregunta?){
            cuestionario = _cuestionario
        }

        fun setBtnRight(_btn: Button) {
            btnRight = _btn
        }

        fun setBtnLeft(_btn: Button) {
            btnLeft = _btn
        }

        override fun onClick(v: View?) {
            val actualBtn = v as Button
            cuestionario?.let { exists ->
                processResponse(testList, exists, modificar = true, respuesta = actualBtn.text.toString())
                if (actualBtn.text == "SI") {
                    btnRight?.isSelected = true
                    btnLeft?.isSelected = false
                } else {
                    btnRight?.isSelected = false
    ...
    inner class TXTEditTextListener: TextWatcher {
        ...
        override fun onTextChanged(s: CharSequence?, start: Int, count: Int, after: Int) {
            ...
            /*
            if (count != 0 && after == 0) { // retrocediendo
            ...
            } else if (after != 0 && count == 0) { // avanzando
                processResponse(testList, cuestionario!!,true, s.toString())
            }
